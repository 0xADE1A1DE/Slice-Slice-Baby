...
def plot_confusion_matrices(averages, stddevs):
    ...
    plt.savefig("llc_rdtscp_NOT_gate_busy_predictions.pdf")
    plt.show()

# Parse the matrices
matrices_busy_rdtscp = parse_confusion_matrix('./current/data_busy_rdtscp.txt')
print(f"Parsed {len(matrices_busy_rdtscp)} confusion matrices.")
matrices_busy_NOT_gate = parse_confusion_matrix('./current/data_busy_NOT_gate.txt')
print(f"Parsed {len(matrices_busy_NOT_gate)} confusion matrices.")

normalised_busy_rdtscp = normalise_to_percentage_per_group(matrices_busy_rdtscp)
normalised_busy_NOT_gate = normalise_to_percentage_per_group(matrices_busy_NOT_gate)

averaged_busy_rdtscp, stddev_busy_rdtscp = calculate_avg_stddev(normalised_busy_rdtscp)
print(averaged_busy_rdtscp)
print(stddev_busy_rdtscp)

averaged_busy_NOT_gate, stddev_busy_NOT_gate = calculate_avg_stddev(normalised_busy_NOT_gate)
print(averaged_busy_NOT_gate)
print(stddev_busy_NOT_gate)

plot_confusion_matrices([averaged_busy_rdtscp, averaged_busy_NOT_gate], [stddev_busy_rdtscp, stddev_busy_NOT_gate])
