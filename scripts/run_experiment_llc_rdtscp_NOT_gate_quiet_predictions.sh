...
FOLDER_PATH="../experiments/llc_rdtscp_NOT_gate_quiet_predictions/current"
...
echo "RDTSCP Scenario: Quiet System"
for (( i = 0; i < 100; i++ )); do
	echo $i
	while true; do
      sudo ASAN_OPTIONS="detect_leaks=0" taskset -c ${THREAD_SIBLINGS_ARRAY[1]} ../build/llc_rdtscp_timings >> $FOLDER_PATH/data_quiet_rdtscp.txt
	    RES=$?
	    if [[ $RES -eq 0 ]]; then
	        break
	    fi
...
echo "NOT Gate Scenario: Quiet System"
for (( i = 0; i < 100; i++ )); do
	echo $i
	while true; do
      sudo ASAN_OPTIONS="detect_leaks=0" taskset -c ${THREAD_SIBLINGS_ARRAY[1]} ../build/llc_NOT_gate_timings >> $FOLDER_PATH/data_quiet_NOT_gate.txt
	    RES=$?
	    if [[ $RES -eq 0 ]]; then
	        break
	    fi
