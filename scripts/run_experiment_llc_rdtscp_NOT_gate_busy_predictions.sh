...
FOLDER_PATH="../experiments/llc_rdtscp_NOT_gate_busy_predictions/current"
...
echo "RDTSCP Scenario: Stress-ng Stress Test (profile=cpu-cache)"
stress-ng --skip-silent --aggressive --all $NUM_CPUS_TO_USE --class cpu-cache --timeout 300s --taskset $CPU_LIST > /dev/null 2>&1 &
sleep 5
for (( i = 0; i < 100; i++ )); do
	echo $i
	while true; do
		sudo ASAN_OPTIONS="detect_leaks=0" taskset -c ${THREAD_SIBLINGS_ARRAY[1]} ../build/llc_rdtscp_timings >> $FOLDER_PATH/data_busy_rdtscp.txt
	    RES=$?
	    if [[ $RES -eq 0 ]]; then
	        break
	    fi
...
killall -INT stress-ng
wait

echo "NOT Gate Scenario: Stress-ng Stress Test (profile=cpu-cache)"
stress-ng --skip-silent --aggressive --all $NUM_CPUS_TO_USE --class cpu-cache --timeout 300s --taskset $CPU_LIST > /dev/null 2>&1 &
sleep 5
for (( i = 0; i < 100; i++ )); do
	echo $i
	while true; do
		sudo ASAN_OPTIONS="detect_leaks=0" taskset -c ${THREAD_SIBLINGS_ARRAY[1]} ../build/llc_NOT_gate_timings >> $FOLDER_PATH/data_busy_NOT_gate.txt
	    RES=$?
	    if [[ $RES -eq 0 ]]; then
	        break
	    fi
