...
FOLDER_PATH="../experiments/llc_COMPARATOR_gate_timings/current"
...
sudo rm -f $FOLDER_PATH/data_*

for CORE in "${CORES[@]}"; do
    echo "Running on core: $CORE"
    while true; do
    	sudo ASAN_OPTIONS="detect_leaks=0" taskset -c $CORE ../build/llc_COMPARATOR_gate_timings >> $FOLDER_PATH/data_${CORE}.txt
        RES=$?
        echo $RES
        if [[ $RES -eq 0 ]]; then
            break
