...
FOLDER_PATH="../experiments/per_core_llc_closest_match_predictions/current"
...
for (( i = 0; i < 100; i++ )); do
    for CORE in "${CORES[@]}"; do
        ...
        while true; do
            sudo ASAN_OPTIONS="detect_leaks=0" taskset -c "$CORE" ../build/slice_partition_closest_match >> "$FOLDER_PATH/data_${FNAME}_${CORE}.txt"
            RES=$?
            if [[ $RES -eq 0 ]]; then
                break
            fi
