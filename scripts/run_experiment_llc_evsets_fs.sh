...
FOLDER_PATH="../experiments/llc_evsets_fs/current"
...
for (( i = 0; i < 1000; i++ )); do
	...
	while true; do
		sleep 1
	    sudo ASAN_OPTIONS="detect_leaks=0" taskset -c ${THREAD_SIBLINGS_ARRAY[1]} ../build/l3_fs >> $FOLDER_PATH/data_$FNAME.txt
	    RES=$?
	    if [[ $RES -eq 0 ]]; then
	        break
	    fi
	...
	while true; do
		sleep 1
	    sudo ASAN_OPTIONS="detect_leaks=0" taskset -c ${THREAD_SIBLINGS_ARRAY[1]} ../build/l3_fs_no_slice >> $FOLDER_PATH/data_no_slice_$FNAME.txt
	    RES=$?
	    if [[ $RES -eq 0 ]]; then
	        break
	    fi
