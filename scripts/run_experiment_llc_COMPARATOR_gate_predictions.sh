...
FOLDER_PATH="../experiments/llc_COMPARATOR_gate_predictions/current"
...
echo "Scenario: Quiet System"
for (( i = 0; i < 100; i++ )); do
	echo $i
	while true; do
		sudo ASAN_OPTIONS="detect_leaks=0" taskset -c ${THREAD_SIBLINGS_ARRAY[1]} ../build/slice_partition >> $FOLDER_PATH/data.txt
	    RES=$?
	    if [[ $RES -eq 0 ]]; then
	        break
	    fi
...
echo "Scenario: Stress-ng Stress Test (profile=cpu-cache)"
stress-ng --skip-silent --aggressive --all $NUM_CPUS_TO_USE --class cpu-cache --timeout 300s --taskset $CPU_LIST > /dev/null 2>&1 &
sleep 5
for (( i = 0; i < 100; i++ )); do
	echo $i
	while true; do
		sudo ASAN_OPTIONS="detect_leaks=0" taskset -c ${THREAD_SIBLINGS_ARRAY[1]} ../build/slice_partition >> $FOLDER_PATH/data_busy.txt
	    RES=$?
	    if [[ $RES -eq 0 ]]; then
	        break
	    fi
